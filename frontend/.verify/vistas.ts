...
ok(contratos.includes('Carlos Ríos') && contratos.includes('María Gómez'), 'lista los arrendatarios sembrados')
...
ok(contratos.includes('Activo') && contratos.includes('Vencido'), 'muestra los estados en espanol')
ok((contratos.match(/Editar/g) ?? []).length === 4, 'una accion Editar por cada uno de los 4 contratos')
...
ok(propiedades.includes('Ciudad') && propiedades.includes('Tipo') && propiedades.includes('Estado'), 'los 3 selectores de filtro estan presentes')
...
ok(transacciones.includes('+ Nueva transaccion') || transacciones.includes('+ Nueva transacción'), 'expone el boton de crear')
ok(transacciones.includes('Tipo') && transacciones.includes('Fuente') && transacciones.includes('Mes'), 'los 3 selectores del issue estan presentes')
...
ok(reportes.includes('Desde (mes)') && reportes.includes('Hasta (mes)'), 'selectores de rango de meses')
...
ok(usuarios.includes('Admin KeyRing') && usuarios.includes('Usuario Demo'), 'lista los 2 usuarios')
ok(usuarios.includes('Administrador') && usuarios.includes('Usuario'), 'muestra los roles en espanol')
ok(!usuarios.includes('admin123') && !usuarios.includes('user123'), 'nunca imprime los passwords reales')
ok(usuarios.includes('Miembro desde'), 'incluye la columna miembro desde')
ok((usuarios.match(/Eliminar/g) ?? []).length === 1, 'el admin de la sesion no tiene boton Eliminar (solo el otro usuario)')
